...
class Uols
  ...
  def unordered_list_tag_open(str_arr)
    str_arr.each_with_index do |str, idx|
      ...
      str.include?("<li>") && prev_line(str_arr, idx, 1) == "\n\n" ? 
  ...
  def unordered_list_tag_close(str_arr)
    str_arr.each_with_index do |str, idx|
      ...
      str.include?("<li>") && prev_line(str_arr, idx, 2).include?("<li>") && 
      (next_line(str_arr, idx, 2).nil? || next_line(str_arr, idx, 2).include?("<li>") != true) ? 
